...
public class WeixinMsgController extends MsgControllerAdapter {
	...
	protected void processInTextMsg(InTextMsg inTextMsg) {
		...
		if ("help".equalsIgnoreCase(msgContent) || "帮助".equals(msgContent)) {
			...
			render(outMsg);
		} else if ("付款".equalsIgnoreCase(msgContent)) {
			OutNewsMsg newsMsg = new OutNewsMsg(inTextMsg);
			List<Meat> meats = meatService.selectAllByState(Meat.STATE_ENABLED);
			for (int i = 0; i < meats.size(); i++) {
				newsMsg.addNews("套餐"+(i+1), meats.get(i).getPrice()+"元"+meats.get(i).getUsetime()+"分钟",
						"https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1513662419660&di=1f4ac0e507e2972d93ed466479736109&imgtype=0&src=http%3A%2F%2Fpic.qjimage.com%2Fchineseview120%2Fhigh%2F214-13701.jpg",
						"http://gesongsyj.free.ngrok.cc/pays?orderuid="+inTextMsg.getFromUserName()+"&menuid="+meats.get(i).getId());
			}
			render(newsMsg);
